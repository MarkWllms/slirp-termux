...
#include <assert.h>
#include "debug.h"
#if __APPLE__ && defined(HAVE_IFADDRS_H)
/* see below */
#include <ifaddrs.h>
#endif
...
#if __APPLE__ && defined(HAVE_IFADDRS_H)
	/* Cameron: This probably works for Linux too, but is needed most on
	   macOS because of .local addresses that may not resolve. */
	if ((len=strlen(buff)) > 6 && !strcmp(buff+len-6, ".local")) 
	{
