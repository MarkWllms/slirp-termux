...
#ifndef FULL_BOLT
	lprint("Slirp-CK v%s (%s)\n\n", SLIRP_VERSION, SLIRP_STATUS);
#else
	lprint("Slirp-CK v%s (%s) FULL_BOLT\n\n", SLIRP_VERSION, SLIRP_STATUS);
#endif

	lprint("Copyright (c) 1995, 2025 Danny Gasparovski, Cameron Kaiser and others.\n");
    ...
    for(++argv;*argv;++argv)
    {
     ...
     if(slirp_tty == NULL) {
    	...
    	*/
		...
		/* Cameron: there's an off by one error here. better to patch
                 * local rather than worry about breaking stuff by changing
                 * strncpy2. */
           env_tty_len = strlen (env_tty) + 1; //;
           slirp_tty = malloc (env_tty_len + 2); //1);
           if (NULL == slirp_tty) {
...
cont_1:
	...
	if ((timeout.tv_usec < 0) || (tmp_time >= 0 && tmp_time < timeout.tv_usec))
		...
		// Cameron
		// https://groups.google.com/g/linux.debian.bugs.dist/c/Ea4GyKTmfrI
		//timeout.tv_usec = (u_int)tmp_time;
#endif
